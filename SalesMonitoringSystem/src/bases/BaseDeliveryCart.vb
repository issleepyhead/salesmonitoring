...
Public Class BaseDeliveryCart
    ...
    Public Sub Update() Implements ICommandPanel.Update
        Try
            _sqlCommand = New SqlCommand("EXEC UpdateDeliveryProcedure @id, @supplier_id, @product_id, @quantity, @delivery_date, @user_id;", _sqlConnection)
    ...
    Public Sub Add() Implements ICommandPanel.Add
        Try
            _sqlCommand = New SqlCommand("EXEC InsertDeliveryProcedure @supplier_id, @product_id, @reference_number, @quantity, @delivery_date, @user_id;", _sqlConnection)
            _sqlCommand.Parameters.AddWithValue("@supplier_id", _data.Item("supplier_id"))
            _sqlCommand.Parameters.AddWithValue("@product_id", _data.Item("product_id"))
    ...
    Public Shared Function FillByProductDelivery(refNo As String) As DataTable
        Try
            Dim conn As SqlConnection = SqlConnectionSingleton.GetInstance
            Dim cmd As New SqlCommand("EXEC FetchProductDeliveryProcedure @ref_no", conn)
    ...
    Public Shared Function Search(query As String) As sgsmsdb.viewtbldeliveriesDataTable
        ...
        Catch ex As Exception
            HandyControl.Controls.MessageBox.Show(ex.Message)
            Return New sgsmsdb.viewtbldeliveriesDataTable
        End Try
    End Function
End Class
